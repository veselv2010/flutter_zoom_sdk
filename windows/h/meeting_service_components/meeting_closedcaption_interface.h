...
public:
	/**
	 * @brief Set the controller event of closed caption(CC).
	 */
	virtual SDKError SetEvent(IClosedCaptionControllerEvent* pEvent) = 0;

	/**
	 * @brief Determine if the current meeting supports the CC feature.
	 */
	...
	/**
	 * @brief Determine if the user can be assigned as a CC sender.
	 */
	...
	/**
	 * @brief Send the CC message.
	 */
	virtual SDKError SendClosedCaption(const zchar_t* ccMsg) = 0;

	/**
	 * @brief Determine if it is enabled to save CC.
	 */
	...
	/**
	 * @brief Get the path of saving CC.
	 */
	virtual const zchar_t* GetClosedCaptionHistorySavedPath() = 0;

	/**
	 * @brief Get the CC URL used by the third party service.
	 */
	 ...
	 * @return If the function succeeds, the return value is SDKErr_Success. Otherwise the function fails and returns an error. To get extended error information, see \link SDKError \endlink enum.
	 */
	 ...
	 * @return If the function succeeds, the return value is the list of the available spoken languages in a meeting. Otherwise failed, the return value is nullptr.	
	 */
	 ...
	 * @return If the function succeeds, the return value is the list of all available translation languages in a meeting. Otherwise failed, the return value is nullptr.
	 */
